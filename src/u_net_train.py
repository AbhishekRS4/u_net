...
from u_net_utils import init, read_config_file, get_tf_dataset
...
def compute_loss(ground_truth, prediction, axis=1, name="mean_cross_entropy"):
    ...
    mean_ce = tf.reduce_mean(
        tf.nn.sparse_softmax_cross_entropy_with_logits(
            labels=ground_truth, logits=prediction
        ), name=name
    )
...
def save_model(session, model_directory, model_file, epoch):
    ...
    saver.save(session,
        os.path.join(os.getcwd(), model_directory, model_file),
        global_step=(epoch + 1)
    )
...
def batch_train(FLAGS):
    ...
    u_net.vgg16_encoder(features)
    print("vgg-16 encoder built")
    u_net.u_net_decoder()
    print("u-net decoder built")
    logits = u_net.logits
    ...
    for epoch in range(FLAGS.num_epochs):
        ...
        for batch_id in range(num_batches_train):
            _, _, loss_per_batch = ss.run([extra_update_op, optimizer_op, loss],
        ...
        print(f"training loss : {temp_train_loss_per_epoch / num_batches_train}, validation loss : {temp_valid_loss_per_epoch / num_batches_valid}, \
