...
class UNet:
    ...
    def u_net_decoder(self):
        self.conv5 = self._get_conv2d_layer(self.pool4, 1024, [1, 1], [1, 1], name="conv5")
        ...
        self.decoder1_out = self._get_decoder_block(self.dropout5, self.conv4_3, 512, name="decoder1_")
        self.decoder2_out = self._get_decoder_block(self.decoder1_out, self.conv3_3, 256, name="decoder2_")
        self.decoder3_out = self._get_decoder_block(self.decoder2_out, self.conv2_2, 128, name="decoder3_")
        self.decoder4_out = self._get_decoder_block(self.decoder3_out, self.conv1_2, 64, name="decoder4_")

        self.logits = self._get_conv2d_layer(self.decoder4_out, self._num_classes, [1, 1], [1, 1], name="logits")

    # return u-net decoder block
    def _get_decoder_block(self, features, features_to_concat, num_kernels, name="decoder_"):
        _conv_tr1 = self._get_conv2d_transpose_layer(features, num_kernels, [2, 2], [2, 2], name=name + "conv_tr")
        _merged_features = tf.concat([features_to_concat, _conv_tr1], axis=self._feature_map_axis, name=name + "concat")

        _conv1 = self._get_conv2d_layer(_merged_features, num_kernels, [3, 3], [1, 1], name=name + "conv1")
    ...
    def _get_conv2d_layer(self, input_tensor, num_filters, kernel_size, strides, name="conv"):
        conv_2d_layer = tf.layers.conv2d(
            inputs=input_tensor, filters=num_filters, kernel_size=kernel_size, strides=strides,
    ...
    def _get_conv2d_transpose_layer(self, input_tensor, num_filters, kernel_size, strides, name="conv_tr"):
        conv_2d_tr_layer = tf.layers.conv2d_transpose(
            inputs=input_tensor, filters=num_filters, kernel_size=kernel_size, strides=strides, padding=self._padding,
    ...
    def _get_elu_activation(self, input_tensor, name="elu"):
        elu_layer = tf.nn.elu(input_tensor, name=name)
        return elu_layer

    # return dropout layer
    def _get_dropout_layer(self, input_tensor, rate=0.5, name="dropout"):
        dropout_layer = tf.layers.dropout(inputs=input_tensor, rate=rate, training=self._is_training, name=name)
        return dropout_layer

    # return batch normalization layer
    def _get_batchnorm_layer(self, input_tensor, name="bn"):
        bn_layer = tf.layers.batch_normalization(input_tensor, axis=self._feature_map_axis, training=self._is_training, name=name)
        return bn_layer
